...
export async function handler(event, context) {
    ...
    // For generateContent, we typically combine the context and user prompt into one string.
    const fullPrompt = `${personality.contextPrompt}\n\nUser Query: "${userPrompt}"\n\n${characterName}'s Response:`;

    console.log(`Netlify Function INFO: Sending to AI (${MODEL_NAME}) for ${characterName}. User prompt: "${userPrompt}"`);
    // console.log(`Netlify Function DEBUG: Full prompt for generateContent: "${fullPrompt}"`);


    try {
        // Using generateContent for a simpler test with Gemini
        const result = await model.generateContent(fullPrompt);
        const response = result.response; // Pausing execution here
        ...
        if (process.env.CONTEXT === 'dev' || process.env.CONTEXT === 'branch-deploy' || process.env.CONTEXT === 'deploy-preview') {
            clientErrorMessage += ` Details: ${error.message || 'Unknown AI error'}`;
        }
